...
export default function Services() {
  ...
  return (
    <section
      id="services"
      className="py-24 overflow-hidden section-divide"
    ...
    >
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="relative">
          {/* Left arrow */}
          <button
            onClick={() => { if (scrollRef.current) scrollRef.current.scrollBy({ left: -400, behavior: 'smooth' }) }}
            className="absolute left-0 top-1/2 -translate-y-1/2 z-20 flex items-center justify-center cursor-pointer transition-all duration-200"
            ...
            onMouseEnter={(e) => { const el = e.currentTarget as HTMLElement; el.style.background = 'rgba(6,182,212,0.25)' }}
            onMouseLeave={(e) => { const el = e.currentTarget as HTMLElement; el.style.background = 'rgba(6,182,212,0.12)' }}
          >
            <ChevronLeft size={20} />
          </button>
          ...
          <button
            onClick={() => { if (scrollRef.current) scrollRef.current.scrollBy({ left: 400, behavior: 'smooth' }) }}
            className="absolute right-0 top-1/2 z-20 flex items-center justify-center cursor-pointer transition-all duration-200"
          ...
          <div
            ...
            style={{ overflowX: 'auto', cursor: 'grab', scrollPadding: '0 24px' }}
            onMouseDown={(e) => {
          ...
          >
            {services.map((svc, i) => (
              <motion.div
                key={svc.number}
                className="flex-none flex flex-col text-center rounded-xl p-8 transition-all duration-300"
                style={{
                  width: 350,
          ...
          {/* Fade edges */}
          <div
            className="absolute top-0 bottom-8 left-0 w-10 pointer-events-none"
        ...
        {/* Scroll hint */}
        <p className="text-center mt-4 text-sm" style={{ color: '#334155' }}>
          ← drag or use arrows to explore all services →
        </p>
        ...
        <motion.div
          ...
          transition={{ duration: 0.6 }}
        >
          <p className="text-sm font-semibold tracking-[0.22em] uppercase mb-6 text-center" style={{ color: '#334155' }}>
