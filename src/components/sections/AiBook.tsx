...
const cards = [
  {
    icon: Bot, label: 'LIVE', labelColor: '#22c55e', title: 'COINsiglieri AI',
    desc: 'An AI-powered Web3 strategist — live and operational. Supports founders and crypto entrepreneurs with strategy, compliance, market analysis, and project management.',
    tags: ['Strategy', 'Compliance', 'Market Analysis', 'Project Management'],
    accent: '#06b6d4', cta: 'Try COINsiglieri AI →',
  },
  {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
          {cards.map((card, i) => {
            ...
            return (
              <motion.div
                key={card.title}
                className="rounded-xl p-10 relative overflow-hidden cursor-default"
                style={{
              ...
              >
                ...
                <div className="flex items-start gap-4 mb-6">
                  ...
                  <div>
                    <span className="font-bold tracking-[0.22em] uppercase" style={{ fontSize: '0.9375rem', color: (card as any).labelColor ?? card.accent }}>
                      {card.label}
                    </span>
                    <h3 className="font-bold mt-1" style={{ fontSize: '1.1875rem', color: '#f8fafc' }}>
                      {card.title}
                    </h3>
                  </div>
                </div>

                <p className="leading-relaxed mb-6" style={{ fontSize: '1rem', color: '#64748b' }}>
                  {card.desc}
                </p>
                ...
                <button
                  className="group/btn inline-flex items-center gap-2 font-semibold cursor-pointer transition-colors duration-200"
                  style={{ fontSize: '1rem', color: card.accent, background: 'none', border: 'none' }}
                >
                  {card.cta}
