...
export default function Media() {
  return (
    <section
      id="media"
      className="py-24 relative section-divide"
      ...
      <div className="max-w-5xl mx-auto px-6">
        ...
        <div className="space-y-2">
          {media.map((item, i) => (
            <motion.div
              key={item.outlet}
              className="group flex items-center gap-6 py-5 px-7 rounded-xl cursor-pointer transition-all duration-200"
              style={{
            ...
            >
              <span
                className="flex-none font-bold tracking-widest uppercase"
                style={{ color: '#22d3ee', minWidth: 140, fontSize: '1.0625rem', fontWeight: 700 }}
              >
                {item.outlet}
              </span>
              <span
                className="leading-relaxed flex-1 truncate"
                style={{ color: '#64748b', fontSize: '1rem', fontWeight: 400 }}
              >
                {item.headline}
              </span>
              <div className="flex items-center gap-3 flex-none">
                <span style={{ color: '#334155', fontSize: '0.875rem' }}>{item.year}</span>
                <ExternalLink
                  size={13}
